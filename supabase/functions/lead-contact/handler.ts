...
export async function handleLead(req: Request): Promise<Response> {
  ...
  try {
    ...
    const { count, error: rateError } = await supa
      .from("lead_requests")
      .select("id", { count: "exact", head: true })
      .eq("ip", ip)
      .gte("created_at", hourAgo);
    if (rateError) return json({ error: "We couldn't send your request. Please try again shortly." }, 503);
    ...
    const { data: row, error: ie } = await supa
      .from("lead_requests")
      .insert({ name, phone, email: email || null, message: enterprise ? `[Enterprise inquiry] ${message}` : message || null, source, ip })
      .select("id")
      .single();
    ...
    const code = enterprise ? null : generateCouponCode();
    ...
    if (owner) {
      await supa.from("notifications").insert({
        ...
        kind: "lead",
        body: enterprise
          ? `${name} requested Filey Enterprise. Review their requirements and contact details.`
          : `${name} requested Filey Freedom. Coupon ${code} (expires ${expiresAt.slice(0, 10)}).`,
    ...
    const res = await fetch("https://api.resend.com/emails", {
      ...
      body: JSON.stringify({
        ...
        subject: `Filey ${plan} — ${name} wants to talk`,
        html:

...
The web app is deployed in Vercel's `GoFiley / filey-erp` project at `https://app.gofiley.com`, with `https://filey-erp.vercel.app` retained as the Vercel address. The website's Open Filey buttons use the custom domain. GoDaddy's CNAME `app` → `84cab2e31d4c788f.vercel-dns-017.com` is saved and verified, and HTTPS serves the production app and its sign-in screen. The apex website and email DNS records were preserved. Public invoice links in the web app use its current origin; no URL environment override is needed.

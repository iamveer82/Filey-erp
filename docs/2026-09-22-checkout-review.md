...
The signed-in Supabase dashboard still lists the Dodo plan/payment secrets but has no `FILEY_AI_OPENROUTER_KEY` or `DODO_AI_CREDIT_PACKS`. Previous deployment notes identify the $5 credit product as a draft. Do not enable credit sales until a funded provider, published products, webhook setup and test-mode payment verification are complete. No provider credentials were added. The Dodo return-handler update was subsequently deployed through the dashboard on 22 September; deployed source matches the tested files and an anonymous status request correctly returns 401. No transaction was made.

## Sync audit

Fresh live metadata matches the column names/types in the audited snapshot for all 17 failing collections. Validation of 4,883 non-null local values found only ten local file-owner placeholders, already converted to the authenticated UUID before upload. No missing column/type migration was established for these failures. Legacy records without revisions, mixed company ownership and missing parent invoice links remain the recovery issues. The additive batch RPC was subsequently deployed and read back on 22 September; customer records and workspace assignments were not modified.

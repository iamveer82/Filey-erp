...
const state = vi.hoisted(() => ({
  ...
  message: (_m: { id: string; from: string; text: string; chatJid?: string; attachment?: { name: string; mimetype: string; b64: string } }) => {},
...
it("attaches an owner's document to a unique turn and returns its generated PDF to the original LID chat", async () => {
  ...
  state.message({ id: "pdf", from: "971500000001", chatJid: "900000000001@lid", text: "Compress this PDF", attachment });
  await answered("pdf");
  ...
  expect(mocks.sendFile).toHaveBeenCalledExactlyOnceWith("900000000001@lid", { path: "C:/Exports/invoice.pdf", filename: "invoice.pdf" });
  expect(mocks.reply).toHaveBeenCalledWith("pdf", expect.stringContaining("invoice.pdf: accepted by WhatsApp"));
...
it("reports an uncertain file send without retrying or logging a success", async () => {
  ...
  expect(mocks.reply).toHaveBeenCalledWith("file-failure", expect.stringContaining("delivery was not confirmed"));
...
it("retains the original attachment for approval and refuses an approval carrying a replacement", async () => {
  ...
  state.message({ id: "attachment-proposal", from: "971500000001", text: "Compress", attachment });
  ...
  state.message({ id: "replacement", from: "971500000001", text: "YES", attachment });
...
it("never forwards produced files after the account changes", async () => {
  ...
  expect(mocks.sendFile).not.toHaveBeenCalled();
  expect(mocks.reply).toHaveBeenCalledWith("changed", "");
});
...
it("handles owner controls without model calls and stops active work before the next request", async () => {
  ...
  expect(signal.aborted).toBe(true);
  await answered("long");
  expect(mocks.reply).toHaveBeenCalledWith("long", "");
  send("new", "/new");
  await answered("new");
...
it("consumes approval for exactly one matching call", async () => {
  ...
  expect(mocks.reply).toHaveBeenCalledWith("proposal", expect.stringContaining('"id": 5'));
  mocks.agent.mockImplementationOnce(async (_messages, opts) => {
...
it("expires waiting turns before they can perform delayed actions", async () => {
  ...
  expect(mocks.agent).toHaveBeenCalledTimes(1);
  expect(mocks.reply).toHaveBeenCalledWith("waiting", expect.stringContaining("expired while waiting"));
});

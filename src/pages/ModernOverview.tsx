...
export default function ModernOverview() {
  ...
  const orderStats = useMemo(() => {
    const count = (statuses: string[]) =>
      orders.filter((o) => statuses.includes((o.status || "").toLowerCase()))
        .length;
    return {
  ...
  const revenue = useMemo(() => {
    ...
    const collected = issued.reduce(
      (s, i) => s + ((i.total || 0) - (i.balance ?? 0)),
      0
    );
          ...
          {loading ? (
            ...
            <>
              <div className="flex items-baseline gap-3 mb-3">
                <span className="text-3xl font-bold font-display text-ink tabular-nums">
                  {orderStats.total}
                </span>
    ...
    <div className="rounded-xl border border-brand-200/80 p-3.5 bg-white shadow-bento dark:border-[#3A3D45] dark:bg-[#24262C] cursor-default">
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0">
          ...
          {sub && <p className="text-[10.5px] text-brand-400 mt-0.5 leading-tight">{sub}</p>}
        </div>
        <div className={cn("rounded-lg p-1.5 shrink-0", toneClass[tone])}>
          {icon}
        </div>
      </div>
    </div>
...
function Mini({ label, value }: { label: string; value: string }) {
  return (
    <div className="rounded-lg bg-brand-50 dark:bg-white/5 px-3 py-2">
      ...
      <p className="text-sm font-bold text-ink tabular-nums mt-0.5 truncate">
        {value}
      </p>
    </div>
  );

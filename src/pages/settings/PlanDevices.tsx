...
/** Device management belongs to billing; activation is automatic. */
export default function PlanDevices() {
  ...
  return (
    ...
    >
      ...
      >
        <Monitor size={15} />
        {busy ? "Checking devices…" : open ? "Hide devices" : "Manage devices"}
      </button>
      {open && (
              ...
              {overview && (
                <div>
                  <p className="text-sm font-medium">
                    Ultra devices ·{" "}
                    {overview.devices.filter((d) => !d.deactivated_at).length} / {LITE_DEVICE_LIMIT}
                  </p>

...
for fname in os.listdir(dir):
    ...
    # Properly split import block: imports start at top and each stmt ends with ;
    lines = content.split('\n')
    import_lines = []
    body_lines = []
    in_imports = True
        ...
        if in_imports:
            if line.strip().startswith('import ') or current_import:
                ...
                if current_import:
                    ...
                    pass  # blank line before imports
            else:
                in_imports = False
                body_lines.append(line)
        else:
            body_lines.append(line)
    # If there's a trailing import without semicolon (shouldn't happen), add it
    if current_import:
        import_lines.extend(current_import)
    body = '\n'.join(body_lines)
